...
from common.write import shorter_path
...
@timeit
def runcmd(command: str, quiet=False, shell=False, parallel=False, dir=None):
    return runcmd_list(command.split(" "), quiet, shell=shell, parallel=parallel, dir=dir)
...
def runcmd_list(command: list, quiet=False, shell=False, parallel=False, dir=None):
    if not quiet:
        for line in " ".join(command).split("\n"):
            dir_str = ""
            if dir:
                dir_str = grey(f" # in {dir}")
            line = shorter_path("$ " + line + dir_str)
            if parallel:
    ...
    stdouts[command_id] = []
    process = subprocess.Popen(command, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=dir)
